export type PreLaunchSchool = {
  id: string
  name: string
  categorySlugs: string[]
  website?: string
  email?: string
  phone?: string | string[]
}

/** Schools contacted before the site existed. Not in the public catalog. */
export const preLaunchSchools: PreLaunchSchool[] = [
  {
    id: 'arte-dance',
    name: 'Studio Arte Dance',
    categorySlugs: ['dance'],
    website: 'https://www.instagram.com/artedance.novisad/',
  },
  {
    id: 'dream-dance',
    name: 'Dream Dance',
    categorySlugs: ['dance'],
    website: 'https://www.facebook.com/p/Dream-Dance-61573529607537/',
  },
  {
    id: 'hattrick',
    name: 'Škola fudbala Hattrick',
    categorySlugs: ['football'],
    email: 'hattrickdoo@gmail.com',
  },
  {
    id: 'radionice-presto',
    name: 'Radionice Presto',
    categorySlugs: ['music', 'art', 'acting', 'dance'],
    website: 'https://www.instagram.com/radionice.presto/',
  },
  {
    id: 'odgajanje-sa-muzikom',
    name: 'Odgajanje sa muzikom',
    categorySlugs: ['music'],
    website: 'https://www.instagram.com/odgajanjesamuzikom.novisad/',
  },
  {
    id: 'skolica-sporta-i-atletike',
    name: 'Školica sporta i atletike',
    categorySlugs: ['mini-sports', 'athletics'],
    website: 'https://www.facebook.com/SkolicaSportaiAtletikeNoviSad/',
  },
  {
    id: 'avalon',
    name: 'Edukativni centar Avalon',
    categorySlugs: ['languages'],
    email: 'info@avalonians.com',
  },
  {
    id: 'naucna-kuhinjica',
    name: 'Naučna kuhinjica Novi Sad',
    categorySlugs: ['science'],
    website: 'https://naucnakuhinjica.com',
    email: 'info@naucnakuhinjica.com',
  },
  {
    id: 'malac-genijalac',
    name: 'Malac Genijalac Novi Sad',
    categorySlugs: ['science'],
    website: 'https://www.facebook.com/malacnovisad',
  },
  {
    id: 'zmajolina',
    name: 'Zmajolina školica sporta',
    categorySlugs: ['kids-sports'],
    website: 'https://zmajolina.com',
    email: 'zmajolinasport@gmail.com',
  },
  {
    id: 'olimp-ladies-kids',
    name: 'Ladies & kids fitness klub Olimp',
    categorySlugs: ['developmental-gymnastics', 'kids-sports'],
    website: 'https://www.facebook.com/ladieskidsolimp/',
    email: 'teretanaolimpnovisad@gmail.com',
    phone: '064 2300 581',
  },
  {
    id: 'akademija-oxford',
    name: 'Akademija Oxford Novi Sad',
    categorySlugs: ['languages'],
    website: 'https://www.instagram.com/akademijaoxfordnovisad/',
  },
  {
    id: 'ns-pro-group',
    name: 'Centar za edukaciju NS Pro Group',
    categorySlugs: ['languages'],
    website: 'https://www.centarzaedukaciju.rs',
    email: 'info@nsprogroup.edu.rs',
  },
  {
    id: 'spartanac-ns',
    name: 'Džudo klub Spartanac NS',
    categorySlugs: ['judo'],
    website: 'https://www.facebook.com/profile.php?id=100049294727475',
  },
  {
    id: 'razvijaliste',
    name: 'Razvijalište',
    categorySlugs: ['kids-sports'],
    website: 'https://www.facebook.com/Razvijaliste/',
    email: 'razvijaliste@gmail.com',
    phone: '063 561 127',
  },
  {
    id: 'dance-code',
    name: 'Dance:Code',
    categorySlugs: ['jazz-ballet', 'ballet'],
    website: 'https://dancecode.org',
    email: 'dancecode.studio@gmail.com',
    phone: '064 109 2800',
  },
  {
    id: 'kliker-centar-grbavica',
    name: 'Kliker Centar Novi Sad – Grbavica',
    categorySlugs: ['programming'],
    website: 'https://klikercentar.rs/location/novi-sad-grbavica/',
    email: 'novisad@klikercentar.rs',
    phone: '065 921 6268',
  },
  {
    id: 'visoko-c',
    name: 'Udruženje umetnika „Visoko C”',
    categorySlugs: ['music', 'acting'],
    website: 'https://branislavcvijic.com/udruzenje-umetnika-visoko-c/',
    email: 'visokoc@yahoo.com',
    phone: '065 677 53 53',
  },
  {
    id: 'iskrice',
    name: 'Muzičko-plesni teatar Iskrice',
    categorySlugs: ['music', 'dance', 'acting'],
    website: 'https://www.facebook.com/p/Muzičko-plesni-teatar-Iskrice-100066582669712/',
    email: 'iskricens@gmail.com',
    phone: '060 3005 552',
  },
  {
    id: 'art-fun-club',
    name: 'Art & Fun Club',
    categorySlugs: ['art'],
    website: 'https://www.facebook.com/artandfunclub/',
    email: 'info@artfun.rs',
    phone: ['064 230 2764', '064 334 0464'],
  },
  {
    id: 'tk-olimp',
    name: 'Taekwondo klub Olimp',
    categorySlugs: ['taekwondo'],
    website: 'https://www.facebook.com/TaekwondoKlubolimpNoviSad',
    email: 'sanjadjuric2009@gmail.com',
    phone: '063 8726 966',
  },
  {
    id: 'tk-dwit',
    name: 'Taekwondo klub Dwit',
    categorySlugs: ['taekwondo'],
    website: 'https://www.facebook.com/profile.php?id=100069225043492',
    email: 'petrovic.92@hotmail.com',
    phone: '065 3391 282',
  },
  {
    id: 'skola-saha-dimitrijevic',
    name: 'Škola šaha Dimitrijević',
    categorySlugs: ['chess'],
    website: 'https://www.facebook.com/skolasahadimitrijevic',
    phone: '062 468 335',
  },
  {
    id: 'skolica-sporta-zmaj',
    name: 'Školica sporta Zmaj',
    categorySlugs: ['kids-sports'],
    website: 'https://www.facebook.com/profile.php?id=100089311495838',
  },
  {
    id: 'helen-doron-ns1',
    name: 'Helen Doron Novi Sad 1',
    categorySlugs: ['languages'],
    website: 'https://www.facebook.com/HelenDoronNoviSad1',
    email: 'novisad1@helendoron.rs',
    phone: ['021 302 92 29', '060 641 60 88'],
  },
  {
    id: 'partizan-2',
    name: 'Partizan 2',
    categorySlugs: ['kids-sports', 'developmental-gymnastics'],
    website: 'https://www.facebook.com/RekreacijaPartizan2Ns',
    email: 'office@partizan2.rs',
    phone: '064 253 80 84',
  },
  {
    id: 'do-re-mi',
    name: 'Do Re Mi muzička radionica',
    categorySlugs: ['music'],
    website: 'https://www.facebook.com/profile.php?id=61593324396421',
    email: 'kontakt@doremi.rs',
    phone: '069 554 43 96',
  },
  {
    id: 'mon-mirage',
    name: 'Plesni klub Mon Mirage',
    categorySlugs: ['dance'],
    website: 'https://www.facebook.com/plesni.klub.Mon.Mirage',
    phone: '060 374 63 27',
  },
  {
    id: 'koda-piano',
    name: 'Koda Piano',
    categorySlugs: ['music'],
    website: 'https://www.facebook.com/koda.piano',
  },
  {
    id: 'da-vinci-novi-sad',
    name: 'Da Vinci Novi Sad',
    categorySlugs: ['programming'],
    website: 'https://www.instagram.com/davinci_novisad/',
  },
  {
    id: 'veslacki-klub-danubius-1885',
    name: 'Veslački klub Danubius 1885',
    categorySlugs: ['swimming'],
    website: 'https://www.facebook.com/p/Veslački-klub-Danubius-1885-100057054026621/',
  },
  {
    id: 'database-training-novi-sad',
    name: 'Database Training Novi Sad',
    categorySlugs: ['programming'],
    website: 'https://www.facebook.com/DatabaseTrainingNoviSad',
  },
  {
    id: 'carobna-frula',
    name: 'Muzička radionica Čarobna frula',
    categorySlugs: ['music'],
    website: 'https://www.facebook.com/muzickaradionicacarobnafrula',
  },
  {
    id: 'svitac-daf',
    name: 'Svitac DaF',
    categorySlugs: ['languages'],
    website: 'https://www.facebook.com/profile.php?id=61581058857543',
  },
  {
    id: 'kreativni-m-kutak',
    name: 'Kreativni M Kutak',
    categorySlugs: ['art'],
    website: 'https://www.facebook.com/KreativniMKutak',
  },
  {
    id: 'ada-comp-ns',
    name: 'Ada Comp NS',
    categorySlugs: ['programming'],
    website: 'https://www.facebook.com/adacompNS',
  },
  {
    id: 'kuglaski-klub-spartak',
    name: 'Kuglaški klub Spartak',
    categorySlugs: ['bowling'],
    website: 'https://www.facebook.com/kuglaskiklub.spartakmatica',
  },
  {
    id: 'ars-nova',
    name: 'Edukativni centar Ars Nova',
    categorySlugs: ['music'],
    website: 'https://www.facebook.com/muzickaskolarsnova',
  },
]
